...
class Wordle:
    ...
    def get_score(self):
        ...
        used_attempts = self.max_attempts - self.attempts
        score_from_attempts = 10 * (self.max_attempts - used_attempts) # 10 points for each attempt you didnt use 
        score_from_word_length = 5 * self.word_length # Get 5 points for each letter in the word
        if used_attempts <= 2:
            first_guess_bonus = 20 # Get 20 extra points if you guessed on the first two tries
        else: 
            first_guess_bonus = 0
        total_score = score_from_attempts + score_from_word_length + first_guess_bonus
        return total_score
...
class User:
    ...
    def add_game_result(self, game):
        ...
        if self.high_score < self.count_high_score():
...
def play_game(user):
    """ This function adds together the elements to play the game """
...
def play_game_loop(user): 
    ...
    while True: # While loop runs until user decides to stop playing, he will then go back to the main menu
        play_game(user)
