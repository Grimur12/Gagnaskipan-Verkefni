...
class Wordle:
    ...
    def game_instructions(self):
        ...
        print("Your total score will depend on how many attempts it took you to guess the word correctly")
...
class User:
    ...
    def add_game(self, score, guesses, status, word, attempts_remaining):
        """ Function that adds the current game to the user profile """
        game_stats = [score, guesses, status, word, attempts_remaining]
        self.games.append(game_stats)

    def add_game_result(self, status):
        """ Function that adds the current games results to the user profile """
        if status == "win": # Add either a loss or a win to the user profile
            self.wins += 1
        else:
            self.losses += 1
    ...
    def save_stats(self):
        ...
        with open("stats.txt", "w") as file:
            ...
            name, wins, losses, total_score, high_score = lines.split(", ")
            if high_score > self.high_score: # Only changing high scores if the current high score is smaller than the new one
    ...
    def save_user_games(self):
        ...
        with open("users.txt", "w") as file:
            for game in self.games: # We are saving the user games to a file by using a for loop and writing the game to the file
                file.write(f"{self.name}, {game[3]}, {game[2]}, {game[4]}\n")
...
def get_attempts():
    try:
        attempts = int(input("Choose the number of attempts you want between 3 and 10: ")) # User chooses the length of the word he wants to guess
    ...
    while True: # While loop maintains until the user decides to quit playing. Enables the user to play the game
        ...
        if choice == "1": # User decided to play Wordle
            while True:
                ...
                user_attempts = get_attempts()
                game = Wordle(user_attempts, word_length) # here we are allowing the user to choose the length of the word
                game.play_round()
                if game.play_again == "y":
                    user.add_game(game.score, game.guesses, game.status, game.word, game.attempts)
                    user.add_game_result(game.status)
                    user.save_stats()
                    game = Wordle(user_attempts, word_length)
                    game.play_round()
                else:
                    break
